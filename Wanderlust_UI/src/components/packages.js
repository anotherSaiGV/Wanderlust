    ...
    render() {        
        if(this.state.bookingPage){
            if(this.state.contact==null){
                ...
                console.log("DA", this.props.bookingPackage);
                
                return(
                    <Redirect to={{pathname: "/book/" + this.props.bookingPackage.destinationId, state: { deal: "D"}}} />                     
                )
            }            
        }
        else{
                                ...
                                {this.displayPackages()}
                                {
                                    this.state.errorMessage ?
                                        (
                                            <div className="offset-md-2">
                                                <h2>Sorry we don't operate in this Destination.</h2><br />
                                                <a href="/packages" className="btn btn-success">Click Here to checkout our Hot Deals</a><br/><br/><br/><br/>    
                                            </div>
                                        )
                                        : null
                                }
                ...
                <Sidebar visible={this.state.showItinerary} position="right" className="p-sidebar-lg" onHide={(e) => this.setState({ showItinerary: false })}>
                    ...
                    <TabView activeIndex={Number(this.state.index)} onTabChange={(e) => this.setState({ index: e.index })}>
                        ...
                        <TabPanel header="Book">
                            ...
                            <div className="text-center">
                                <button disabled={!this.state.bookActive} className="btn btn-success" onClick={() => this.openBooking(this.state.deal, this.state.bookingForm, this.state.totalCharges, this.state.checkOutDate, this.state.continent)}>Book</button>
                                &nbsp; &nbsp; &nbsp;
